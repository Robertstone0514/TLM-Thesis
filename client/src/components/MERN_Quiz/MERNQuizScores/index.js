...
function MERNQuizScores({
  ...
  const [currentUserScoreData, setUserScoreData] = useState([])
  ...
  const getPercentage = async (userScoreData) => {
    ...
    const setMERNProgressScores = await (await api.setMERNProgressScore({ ...payload, newTotalPercent }, userData._id));
    if (!setMERNProgressScores.data.success) {
      /* eslint-disable-next-line no-console */
      console.log('There was an error setting progress scores!');
    }
    const { highestScore, passed } = userData.gameStatus.MERN;
    ...
    if (newTotalPercent > userData.gameStatus.MERN.highestScore) {
      setMERNHighScore(highScorePayload);
    }
    const setMERNQuizData = await (await api.setMERNQuizData({ ...highScorePayload }, userData._id));
    if (!setMERNQuizData.data.success) {
      /* eslint-disable-next-line no-console */
      console.log('There was an error with the MERN Quiz Data Set')
    }
  ...
  const languagePercent = (num, correct, lang) => {
    let percentLogic = 0
    if (num > 0) {
      percentLogic = Math.round(((correct.filter((question) => question.language === lang).length) / totalQuestions.react) * 100)
    }
    return percentLogic
  }
  ...
  return (
    <Container fluid className="p-0">
      <h1>You have completed the MERN Stack Quiz</h1>
...
export default connect(({ userData }) => ({ userData }), null)(MERNQuizScores);
